...
print("=" * 70)
print("FAIL-CLOSED TEST PROCEDURE (per ENGINE LOCK spec Section 1.4):")
print("=" * 70)
print()

print("STEP 1: Verify simulation EXISTS (should return APPROVED/BLOCKED)")
print(f"curl -s 'https://beta.beatvegas.app/api/games/{league}/{event_id}/decisions' | jq '.spread.release_status, .spread.classification'")
print()
...
print("STEP 3: Verify FAIL-CLOSED response (per spec - HTTP 200, not 503!)")
print(f"  curl -s 'https://beta.beatvegas.app/api/games/{league}/{event_id}/decisions' | jq '.spread.release_status, .spread.classification, .spread.risk.blocked_reason'")
print()
print("  Expected per spec Section 1.4:")
print("    - HTTP 200 (not 503!)")
print("    - release_status = 'BLOCKED_BY_MISSING_DATA'")
print("    - classification = null")
print("    - risk.blocked_reason = 'sharp_analysis.spread.model_spread'")
print()
