"""
NCAAB (NCAA Basketball) Calibration & Edge Logic

Spread-first sport with higher variance than NBA.
Compression factor: 0.80 (most conservative)
Blowout risk requires larger spreads.
"""
from typing import Dict, Optional, Tuple
from dataclasses import dataclass
from .sport_configs import (
    NCAAB_CONFIG, EdgeState, MarketType, DistributionFlag, VolatilityLevel
)


@dataclass
class NCAABMarketEvaluation:
    """Result of evaluating a specific NCAAB market"""
...
def compress_probability(raw_prob: float, compression_factor: float = 0.80) -> float:
    ...
    NCAAB uses most aggressive compression (0.80) due to high variance
...
def calculate_spread_edge(
    ...
    Args:
        sim_win_prob: Simulated probability of covering the spread
...
def check_spread_size_guardrails(spread: float) -> Tuple[bool, bool, Optional[str]]:
    """
    ...
    max_fav_spread = config.max_favorite_spread or 0.0
    if abs_spread > max_fav_spread:
        return True, False, f"SPREAD_TOO_LARGE_{abs_spread}"
    
    # Flag as large spread if above threshold
    is_large_spread = abs_spread > 10.0
...
def assess_distribution_volatility(
    ...
    NCAAB has higher baseline volatility than NBA
    """
    if market_type == MarketType.SPREAD:
    ...
    # Volatility classification (adjusted for college basketball variance)
    if std < 0.025:
...
def check_eligibility_gates(
    ...
    if market_type == MarketType.SPREAD:
        ...
        if spread is not None:
            _, eligible, reason = check_spread_size_guardrails(spread)
            if not eligible:
                return False, reason
...
def evaluate_ncaab_market(
...
) -> NCAABMarketEvaluation:
    """
    Complete evaluation of an NCAAB market
    ...
    if market_type == MarketType.SPREAD and spread is not None:
        ...
        is_large_spread, spread_eligible, spread_block_reason = check_spread_size_guardrails(spread)
        
        if not spread_eligible:
            return NCAABMarketEvaluation(
    ...
    if market_type == MarketType.SPREAD:
        ...
        raw_edge, compressed_edge = calculate_spread_edge(
            sim_cover_prob, spread, spread_odds, NCAAB_CONFIG.compression_factor
        )
    elif market_type == MarketType.TOTAL:
        ...
        side, raw_edge, compressed_edge = calculate_total_edge(
            sim_over_prob, over_odds, under_odds, NCAAB_CONFIG.compression_factor
        )
    else:
        return NCAABMarketEvaluation(
            market_type=market_type,
