...
interface SidebarProps {
  ...
  isAdmin?: boolean; // NEW: Pass admin status to show admin panel link
  phase11Complete?: boolean; // Gate Affiliates nav until Phase 11 is complete
}
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, onLogout, userRole = 'user', isAdmin = false, phase11Complete }) => {
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);
  // Affiliates nav is gated behind Phase 11 completion flag
  const showAffiliates = phase11Complete ?? (import.meta as any).env?.VITE_PHASE11_COMPLETE === 'true';
  // For Developers tab is only visible when VITE_SIMSPORTS_LIVE=true
  // Defaults to false — never visible in production unless explicitly enabled
  ...
  return (
    <>
      ...
      <nav className="flex-1 flex flex-col space-y-2 pt-4">
        ...
        {/* War Room Section */}
        <div className="pt-2 border-t border-navy/50 mt-2">
          <p className="px-4 text-xs font-semibold text-electric-blue/70 uppercase tracking-wider mb-2">⚔️ War Room</p>
          <NavLink page="war-room" currentPage={currentPage} setCurrentPage={setCurrentPage} icon={<WarRoomIcon />}>Live Room</NavLink>
          <NavLink page="war-room-leaderboard" currentPage={currentPage} setCurrentPage={setCurrentPage} icon={<LeaderboardIcon />}>Rankings</NavLink>
        </div>
        ...
        {/* For Developers — only visible when VITE_SIMSPORTS_LIVE=true; hidden in production */}
        {showDevTab && (
          <NavLink page="developers" currentPage={currentPage} setCurrentPage={setCurrentPage} icon={<DevIcon />}>For Developers</NavLink>
        )}
        ...
        <div className="pt-4 border-t border-navy mt-2">
            ...
            <NavLink page="telegram" currentPage={currentPage} setCurrentPage={setCurrentPage} icon={<TelegramIcon />}>Telegram</NavLink>
            
            {/* Admin Panel - Only visible for admin users */}
            {isAdmin && (
              <div className="pt-2 border-t border-purple-500/30 mt-2">
                <NavLink page="admin" currentPage={currentPage} setCurrentPage={setCurrentPage} icon={<AdminIcon />}>
                  <span className="flex items-center gap-2">
                    Admin Panel
                    <span className="text-xs bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded">Admin</span>
                  </span>
                </NavLink>
              </div>
            )}
...
const WarRoomIcon = () => <IconWrapper><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></IconWrapper>;
...
const TelegramIcon = () => <IconWrapper><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></IconWrapper>;
const AdminIcon = () => <IconWrapper><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></IconWrapper>;
const DevIcon = () => <IconWrapper><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></IconWrapper>;
